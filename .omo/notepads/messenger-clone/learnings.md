...
# Task 15 Learnings — Title Screen + Opening Dialogue Cutscene (src/intro.ts)
...
## Key Decisions
- **State machine: title → intro → playing (3 states)** — `state` is a closure variable, initially `'title'`. `beginGame()` transitions title→intro (after AudioManager.resume + hideTitle). `showOpeningDialogue()`'s onComplete transitions intro→playing + calls `onStart()`. The `beginGame()` guard `if (state !== 'title') return` prevents double-trigger from simultaneous button click + window keydown (same pattern as Task 8's blur() double-fire fix, but using state guard instead).
- **Dynamic import for showDialogue (`await import('./ui/dialogue')`)** — Same pattern as Task 14 (interaction.ts). dialogue.ts imports `./dialogue.css` which tsx cannot parse (Task 8 gotcha). A top-level `import { showDialogue }` would make the mandatory `npx tsx -e "import {createIntro} from './src/intro'"` fail. The dynamic import only executes when `beginGame()` is called (browser only, after user gesture). In tsx verification, `show()` is never called, so the dynamic import never triggers. try/catch fallback: if dialogue fails to load, skip straight to `state='playing'` + `onStart()`.
- **Styles JS-injected, NOT CSS import** — Same pattern as Task 11 (quest.ts) and Task 14 (interaction.ts). `ensureStyles()` injects a `<style>` element via `document.createElement('style')` once, guarded by `stylesInjected` flag. Self-contained module, tsx-compatible. The title CSS references `var(--ui-font, 'Nunito', ...)` to reuse the design token from dialogue.css if loaded; otherwise the fallback applies.
- **`typeof document !== 'undefined'` guard on show() + ensureStyles()** — In Node (tsx verification), `document` is undefined. The guard makes `show()` a no-op in Node while `getState()` still returns `'title'`. This is what makes the verification command work: `createIntro(()=>{})` creates the controller, `getState()` returns `'title'`, `show()` is a no-op.
- **Begin button: autofocus + window keydown listener (dual input)** — The `<button>` is natively focusable and responds to Enter/Space when focused. `beginBtn.autofocus = true` + `requestAnimationFrame(() => beginBtn.focus())` ensures keyboard works immediately on page load. A window keydown listener (Enter/Space) is also registered as a fallback for when the button loses focus. The state guard (`if (state !== 'title') return`) prevents double-trigger if both fire. `beginBtn.blur()` in the click handler is belt-and-suspenders (Task 8 pattern).
- **`requestAnimationFrame` for focus** — Setting focus immediately after `appendChild` can fail in some browsers (the element isn't fully laid out yet). `requestAnimationFrame(() => beginBtn.focus())` defers to the next paint frame, ensuring the button is renderable before focusing. This is the standard pattern for programmatic focus on newly-created elements.
- **Title screen aesthetic: cyan gradient + glow + white MESSENGER + yellow Begin** — Evokes the original game's "青色水面 + 圆岛 + 白色 MESSENGER + 黄 Begin" without complex 2D illustration (per MUST NOT DO). Linear gradient (#7dd3fc → #0891b2) suggests water depth. Radial gradient glow behind title adds atmosphere. Yellow button (#fbbf24) with 3D press effect (box-shadow bottom border `0 6px 0 #d97706`) + pulsing glow animation. The 3D press effect (translateY + reduced box-shadow on :active) gives tactile feedback.
- **`clamp()` for responsive typography** — `font-size: clamp(48px, 13vw, 148px)` for the title scales with viewport but has min/max bounds. Prevents the title from being too small on mobile or absurdly large on ultra-wide displays. Same pattern for button (`clamp(18px, 3vw, 22px)`) and subtitle (`clamp(12px, 2vw, 15px)`).
- **INTRO_LINES module-private (not exported)** — The 2 dialogue lines are a constant inside the module. They're not exported because they're only used by `showOpeningDialogue()`. The exact text is verified via the evidence file + browser QA (Playwright), not via tsx. Exporting them would be unnecessary API surface.
- **`AudioManager` top-level import is tsx-safe** — Task 4 verified that importing audio.ts in tsx works (Howler loads in Node without throwing). `AudioManager.resume()` is only called from `beginGame()` which is browser-only, so no audio side effects in tsx verification.

## Verification Results
- `npx tsc --noEmit`: exit 0 (strict, noUnusedLocals/Parameters clean).
- `lsp_diagnostics` on intro.ts: no diagnostics.
- `npx tsx -e "import {createIntro} from './src/intro'; const i=createIntro(()=>{}); console.log('state:', i.getState())"`: `state: title`, exit 0.
- Structural test: `has_show=true`, `has_getState=true`, `state=title`, `state_after_show=title` (show() no-op in Node), `EXIT_OK`.
- Evidence: `.omo/evidence/task-15-intro.txt`.

## Gotchas
- **tsx CSS import chain incompatibility (CRITICAL)** — Same as Task 8/11/14: dialogue.ts imports dialogue.css which tsx cannot parse. Must use dynamic `import('./ui/dialogue')` inside a function body, never a top-level static import. This is now the 3rd task (after 14, 15) using this pattern. Future tasks (16) that import dialogue.ts or quest.ts should use the same pattern if they have tsx verification commands.
- **Button focus + window keydown double-fire** — When the Begin button is focused and Enter/Space is pressed, BOTH the button's native activation (click event) AND the window keydown listener fire. Without protection, `beginGame()` would be called twice. Two layers of protection: (1) `beginBtn.blur()` in the click handler (Task 8 pattern), (2) `if (state !== 'title') return` state guard in `beginGame()`. Either alone suffices; both together are belt-and-suspenders.
- **`beginBtn.autofocus = true` is not always reliable** — The HTML `autofocus` attribute is a hint, not a guarantee. Some browsers ignore it on dynamically-created elements. The `requestAnimationFrame(() => beginBtn.focus())` is the reliable fallback. Both are set for robustness.
- **Dialogue text appears lowercase despite uppercase data** — dialogue.css has `text-transform: lowercase` on `.dialogue-text` (Task 8 design decision for NPC dialogue convention). The INTRO_LINES data is uppercase (as specified), but it will display as lowercase in the dialogue UI. This is consistent with the original game's convention and the dialogue system's design. The speaker "messenger" displays as "MESSENGER" (uppercase via `.dialogue-name-tag` CSS). This is not a bug — it's the dialogue system working as designed.
- **No `src/ui/title.css` file created** — The plan mentioned "创建 `src/intro.ts` + `src/ui/title.css`" but per the MUST DO constraint "样式 JS 注入（兼容 tsx，无 CSS import）或内联 style", styles are JS-injected. This is the correct trade-off: a separate CSS file would require a CSS import in intro.ts, which would break the mandatory `npx tsx -e` verification (same as Task 11/14). The plan's mention of title.css is overridden by the MUST DO constraint.
- **Did NOT touch main.ts/scene.ts** (G17 preserveDrawingBuffer lock preserved; integration deferred to Wave 4 per task constraint).

# Task 16 Learnings — UI & HUD (src/ui/hud.ts)

## Files Added
- `src/ui/hud.ts` — exports `createHud(questManager)` → `{update, show, hide, isPaused}`. ~280 lines (incl. injected CSS string). No changes to main.ts/scene.ts/other task files.

## Key Decisions
- **Styles JS-injected, NOT CSS import** — Same pattern as Task 11 (quest.ts), Task 14 (interaction.ts), Task 15 (intro.ts). The mandatory `npx tsx -e` verification would fail with CSS imports (Task 8/11 gotcha). `ensureStyles()` injects a `<style>` element once, guarded by `stylesInjected` flag. Self-contained module, tsx-compatible.
- **`QuestManagerLike` structural interface** — The verification command passes `{getQuests:()=>[]}` (a plain object, not a `QuestManager` instance). The full `QuestManager` class has `startQuest`/`advanceStep`/`completeQuest`/`isComplete`/`showObjective`/`showCompletion` methods, so a plain object with only `getQuests()` would NOT satisfy the `QuestManager` type. Solution: define a local `QuestManagerLike` interface with only `getQuests()`. TypeScript structural typing means both the real `QuestManager` instance and test doubles satisfy this interface.
- **`isPaused` as a getter, not a plain property** — Same pattern as Task 14's `isDialogueActive`. `get isPaused() { return paused; }` in the return object ensures the property always reflects the current closure state. A plain property `{ isPaused: false }` would be a snapshot at creation time (always false). The `Hud` interface declares `readonly isPaused: boolean` — the getter inherently satisfies `readonly`.
- **`update()` caches `lastObjective` to avoid per-frame DOM writes** — `update()` is called every frame. Without caching, it would call `objectiveEl.textContent = text` every frame even when the text hasn't changed, causing unnecessary DOM mutations. The `if (text === lastObjective) return;` guard ensures DOM writes only happen when the objective actually changes. `lastObjective` is reset to `''` in `hide()` so the next `show()` + `update()` cycle re-syncs.
- **Dark semi-transparent HUD elements (rgba(15,23,42,0.72) + backdrop-filter blur)** — Unlike the dialogue box (white bg, bottom of screen), HUD elements are in the top-right corner against variable game backgrounds (sky, ground, NPCs). Dark semi-transparent pills with white icons/text are readable against any background. The `rgba(15,23,42,0.72)` matches the quest overlay panel color (Task 11) for visual consistency. `backdrop-filter: blur(8px)` adds a frosted-glass effect.
- **Mute indicator via CSS `::after` diagonal line** — Instead of swapping icons (♪ ↔ 🔇), the sound button always shows ♪. When muted, `.hud-btn--muted` class adds a `::after` pseudo-element with a red diagonal line (`transform: rotate(-45deg)`). This is more reliable than emoji rendering (which varies by OS/browser) and cleaner than swapping Unicode characters. The red color (#ef4444) clearly signals "off/disabled".
- **`pointer-events: none` on root, `auto` on buttons** — Same pattern as dialogue.ts (Task 8). The HUD root container doesn't block the game canvas. Only the button row has `pointer-events: auto` (clickable). The objective text inherits `none` (non-interactive, display-only). `pointer-events` inherits by default in CSS.
- **Pause menu z-index 120 (above dialogue z-100)** — The pause menu must overlay everything, including active dialogue. z-index hierarchy: game canvas (0) < HUD root (80) < quest overlay (90) < dialogue overlay (100) < pause menu (120).
- **Quit to title via `window.location.reload()`** — Can't modify main.ts (task constraint) and can't call intro.ts's `show()` directly (would need a reference to the intro controller). `window.location.reload()` restarts the page, which re-runs main.ts and (in Wave 4 integration) shows the title screen again. This is the standard web game pattern for "quit to title" when the game loop can't be restructured.
- **Esc keydown listener registered in `show()`, removed in `hide()`** — The listener is stored in `keydownHandler` closure variable. `show()` creates and registers it; `hide()` removes and nulls it. This prevents listener leaks if `show()`/`hide()` are called multiple times. `show()` also calls `hide()` first (cleanup any existing instance) — same pattern as `showDialogue` in dialogue.ts.
- **`typeof document !== 'undefined'` guard on `show()`/`openPauseMenu()`** — In Node (tsx verification), `document` is undefined. The guard makes `show()` a no-op in Node while `createHud()` + `isPaused` getter still work. Same pattern as Task 11/14/15.
- **Design tokens reused via `var(--token, fallback)`** — All CSS references use `var(--ui-font, 'Nunito', ...)`, `var(--ui-blue, #2563eb)`, `var(--ui-bg, #ffffff)`, `var(--ui-radius, 14px)`, etc. If dialogue.css (Task 8) is loaded, the shared tokens are used; otherwise the fallbacks apply. Decouples hud.ts from dialogue.css while maintaining visual consistency.
- **No `src/ui/hud.css` file created** — The plan mentioned "创建 `src/ui/hud.ts` + `src/ui/hud.css`" but per the MUST DO constraint "样式 JS 注入（兼容 tsx）或内联 style", styles are JS-injected. Same override as Task 15 (title.css). A separate CSS file would require a CSS import, breaking tsx verification.

## Verification Results
- `npx tsc --noEmit`: exit 0 (strict, noUnusedLocals/Parameters clean).
- `lsp_diagnostics` on hud.ts: no diagnostics.
- Spec verification: `npx tsx -e "import {createHud} from './src/ui/hud'; const h=createHud({getQuests:()=>[]}); console.log('paused:', h.isPaused, 'ok')"` → `paused: false ok`, exit 0.
- API structure test: has_show/has_hide/has_update/has_isPaused all true; isPaused=false; update_no_throw=true; hide_no_throw=true.
- Quest lookup logic: active quest_0 step 0 → "FIND THE RED CLIFF HOUSE AND GET THE LETTER BACK" (matches quests.ts data).
- Evidence: `.omo/evidence/task-16-hud.txt`.

## Gotchas
- **`pointer-events` inherits by default** — Setting `pointer-events: none` on `.hud-root` makes ALL children non-interactive unless explicitly overridden. Must set `pointer-events: auto` on `.hud-buttons` (or each button) to make them clickable. The objective text div intentionally inherits `none` (non-interactive).
- **`display: none` in CSS + inline style override** — `.hud-objective` has `display: none` in CSS (hidden by default). `show()` sets `objectiveEl.style.display = 'none'` (redundant but explicit). `update()` sets `objectiveEl.style.display = 'block'` to show, `'none'` to hide. Using `''` (empty string) would fall back to CSS `none`, which also works for hiding but is less explicit. Using `'block'` to show is unambiguous.
- **Button `blur()` after click** — Same pattern as Task 8 (dialogue advance button). After clicking a HUD button, it retains focus. Subsequent Enter/Space would activate the focused button (e.g., reopening pause menu immediately after closing it). `blur()` prevents this. Belt-and-suspenders with the `paused` state guard in `openPauseMenu()`.
- **`menuBtn` is a local variable, not a closure variable** — `menuBtn` is only needed during `show()` setup (attach event listener + append to DOM). It doesn't need to be referenced after `show()` returns. `soundBtn` IS a closure variable because `toggleMute()` needs to update its class. `objectiveEl` IS a closure variable because `update()` needs to update its content. This distinction avoids unnecessary closure variable pollution.
